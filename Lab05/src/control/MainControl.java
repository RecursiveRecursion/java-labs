...
public class MainControl {
    ...
    private void createModal() throws Exception{
        ...
        promptControl = promptLoader.getController();
        //checkCurrentSelection();
    }
    ...
    private void transfer() {
        try {
            createModal();
            promptControl.transfer(currentAccount);
            modal.showAndWait();
    ...
    private void topup() {
        try{
            createModal();
            promptControl.topup(currentAccount);
            modal.showAndWait();
            VAccountsList.refresh();
        }
        catch (Exception e) {
    ...
    private void withdraw() {
        try{
            createModal();
            promptControl.withdraw(currentAccount);
            modal.showAndWait();
            VAccountsList.refresh();
        }
        catch (Exception e) {
    ...
    private void closeAccount(){
        try{
            createModal();
            promptControl.closeAccount(currentAccount);
            modal.showAndWait();
            VAccountsList.getSelectionModel().clearSelection();
            currentAccount = null;
